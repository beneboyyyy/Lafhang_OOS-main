...
if (isset($_POST['multisave'])) {
    ...
    // Getting the account information if the form is submitted
    if(isset($_POST['cust_user']) && isset($_POST['cust_email']) && isset($_POST['cust_pass'])) {
        // Getting the account information
        $username = $_POST['cust_user'];
        $email = $_POST['cust_email'];
        $password = password_hash($_POST['cust_pass'], PASSWORD_BCRYPT);
    } else {
        $error = "Account information is missing.";
    }

    // Getting the personal information if the form is submitted
    if(isset($_POST['cust_FN']) && isset($_POST['cust_LN']) && isset($_POST['cust_phone'])) {
    ...
    // Getting the address information if the form is submitted
    if(isset($_POST['addressline1']) && isset($_POST['barangay_text']) && isset($_POST['city_text']) && isset($_POST['region_text'])) {
        $street = $_POST['addressline1'];
        $barangay = $_POST['barangay_text'];
        $city = $_POST['city_text'];
        $province = $_POST['region_text'];
    } else {
        $error = "Address information is missing.";
    }

    // Handle file upload if the form is submitted
    if(isset($_FILES["cust_image"]) && isset($_FILES["cust_image"]["name"])) {
        $target_dir = "uploads/";
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["cust_image"]["tmp_name"], $target_file)) {
                ...
                $userimage = 'uploads/'.$new_file_name; // Save the new file name (without directory)
                
                $userID = $con->signupUser($firstname, $lastname, $email, $username, $password,$phone, $userimage);

                if ($userID) {
                    // Signup successful, insert address into users_address table
                    if ($con->insertAddress($cust_id,$street, $barangay, $city, $province,$Zipcode)) {
    ...
    <div class="form-step form-step-active" id="step-1">
      <div class="card mt-4">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="cust_user" id="cust_user" placeholder="Enter username" required>
          ...
          <div class="form-group">
            ...
            <input type="email" class="form-control" name="email" id="cust_email" placeholder="Enter email" required>
            <div class="valid-feedback">Looks good!</div>
          ...
          <div class="form-group">
            ...
            <input type="password" class="form-control" name="password" id="cust_pass" placeholder="Enter password" required>
            <div class="valid-feedback">Looks good!</div>
          ...
          <div class="form-group">
            ...
            <input type="password" class="form-control" name="confirmPassword" id="cust_pass" placeholder="Re-enter your password" required>
            <div class="valid-feedback">Looks good!</div>
